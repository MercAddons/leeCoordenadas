...
openni::Status SampleViewer::Init(int argc, char **argv)
{
   ...
   colorVideo.create(m_device,openni::SENSOR_COLOR);
   colorVideo.start();
...
void DrawLimb(nite::UserTracker* pUserTracker, const nite::SkeletonJoint& joint1, const nite::SkeletonJoint& joint2, int color,bool b=false)
{
	float coordinates[6] = {0};
   if(b){
      printf("[%f,%f,%f] ",joint1.getPosition().x,joint1.getPosition().y,joint1.getPosition().z);
	...
	coordinates[4] *= GL_WIN_SIZE_Y/(float)g_nYRes;
   if(b){
      printf("[%f,%f] ", coordinates[0],coordinates[1]);
...
{
	DrawLimb(pUserTracker, userData.getSkeleton().getJoint(nite::JOINT_HEAD), userData.getSkeleton().getJoint(nite::JOINT_NECK), userData.getId() % colorCount,true);
...
/* PRUEBA DE DETECTAR CIRCULOS CON HOUGH CIRCULOS.
   Va demasiado lento
   Demasiados círculos.
* /
  ...
  cv::Mat frame, frameDetect;
   ...
   if(m_device.getSensorInfo(openni::SENSOR_COLOR) != NULL){
      ...
      state=colorVideo.readFrame(&colorFrame);
      if(colorFrame.isValid()){
         vmode = colorVideo.getVideoMode();
         heightColor=colorFrame.getHeight();
         widthColor=colorFrame.getWidth();
         frame.create(heightColor,widthColor,CV_8UC3);
         frame.data=(uchar*) colorFrame.getData();
         cv::cvtColor(frame,frame,CV_BGR2RGB);
         cv::cvtColor(frame,frameDetect,CV_BGR2GRAY);
         cv::GaussianBlur(frameDetect,frameDetect,cv::Size(9,9),2,2);
         std::vector<cv::Vec3f> circles;
         cv::HoughCircles(frameDetect,circles,CV_HOUGH_GRADIENT,2,frameDetect.rows/4, 200,100);

         for(size_t i=0; i<circles.size(); i++){
            cv::Point center(cvRound(circles[i][0]),cvRound(circles[i][1]));
            int radius =cvRound(circles[i][2]);
            circle(frameDetect,center,3,cv::Scalar(0,255,0),-1,8,0);
            circle(frameDetect,center,radius,cv::Scalar(0,0,255),3,8,0);
         }
         cv::resize(frameDetect,frameDetect,cv::Size(640,480));
         cv::namedWindow("color",1);
         cv::imshow("color",frameDetect);
         cv::waitKey(1);
      }
   }
